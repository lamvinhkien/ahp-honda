...
hieunang_bp = Blueprint("hieunang", __name__, url_prefix="/hieu-nang")
...
@hieunang_bp.route("/", methods=["GET", "POST"])
def hieunang_page():
    criteria_name = "Hiệu năng"
    criteria = Criteria.query.filter_by(name=criteria_name).first()
    honda_types = LaptopType.query.all()
    selected_laptop_type_id = request.form.get('selected_laptop_type_id') or request.args.get('selected_laptop_type_id') or session.get('hieunang_selected_laptop_type_id')
    selected_laptop_type = LaptopType.query.get(selected_laptop_type_id)
    alternatives = []
    if selected_laptop_type_id:
        session['hieunang_selected_laptop_type_id'] = selected_laptop_type_id
        alternatives = Alternatives.query.filter_by(laptop_type_id=selected_laptop_type_id).all()

    weights = session.get('hieunang_weights')
    cr = session.get('hieunang_cr')
    error = None
    submitted_values = session.get('hieunang_comparison_values', {})
    input_errors = {}
    ...
    if request.method == "POST" and criteria and selected_laptop_type:
        ...
        session['hieunang_comparison_values'] = submitted_values_from_form
        ...
        if valid_input:
            ...
            cr = cr_calculated
            session['hieunang_weights'] = weights
            session['hieunang_cr'] = cr
            session['alternative_weights'] = session.get('alternative_weights', {})
            if weights:
                session['alternative_weights']['hieunang'] = dict(zip(alternative_names, weights))
            else:
                if 'hieunang' in session['alternative_weights']:
                    del session['alternative_weights']['hieunang']
        ...
        comparison_matrix_data = generate_alternative_comparison_matrix_data(alternatives, criteria.id, selected_laptop_type_id, load_suggestions, session.get('hieunang_comparison_values'))
    else:
