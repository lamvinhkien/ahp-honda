...
tgbh_bp = Blueprint("tgbh", __name__, url_prefix="/tgbh")
...
@tgbh_bp.route("/", methods=["GET", "POST"])
def tgbh_page():
    ...
    if selected_laptop_type_id:
        session['tgbh_selected_laptop_type_id'] = selected_laptop_type_id
        alternatives = Alternatives.query.filter_by(laptop_type_id=selected_laptop_type_id).all()

    weights = session.get('tgbh_weights')
    cr = session.get('tgbh_cr')
    error = None
    submitted_values = session.get('tgbh_comparison_values', {})
    input_errors = {}
    ...
    if request.method == "POST" and criteria and selected_laptop_type:
        ...
        session['tgbh_comparison_values'] = submitted_values_from_form
        ...
        if valid_input:
            ...
            cr = cr_calculated
            session['tgbh_weights'] = weights
            session['tgbh_cr'] = cr
            session['alternative_weights'] = session.get('alternative_weights', {})
            if weights:
                session['alternative_weights']['tgbh'] = dict(zip(alternative_names, weights))
            else:
                if 'tgbh' in session['alternative_weights']:
                    del session['alternative_weights']['tgbh']
