...
tienich_bp = Blueprint("tienich", __name__, url_prefix="/tien-ich")
...
@tienich_bp.route("/", methods=["GET", "POST"])
def tienich_page():
    criteria_name = "Tiện ích"
    criteria = Criteria.query.filter_by(name=criteria_name).first()
    honda_types = HondaType.query.all()
    selected_honda_type_id = request.form.get('selected_honda_type_id') or request.args.get('selected_honda_type_id') or session.get('tienich_selected_honda_type_id')
    selected_honda_type = HondaType.query.get(selected_honda_type_id)
    alternatives = []
    if selected_honda_type_id:
        session['tienich_selected_honda_type_id'] = selected_honda_type_id
        alternatives = Alternatives.query.filter_by(honda_type_id=selected_honda_type_id).all()

    weights = session.get('tienich_weights')
    cr = session.get('tienich_cr')
    error = None
    submitted_values = session.get('tienich_comparison_values', {})
    input_errors = {}
    ...
    if request.method == "POST" and criteria and selected_honda_type:
        ...
        session['tienich_comparison_values'] = submitted_values_from_form
        ...
        if valid_input:
            ...
            cr = cr_calculated
            session['tienich_weights'] = weights
            session['tienich_cr'] = cr
            session['alternative_weights'] = session.get('alternative_weights', {})
            if weights:
                session['alternative_weights']['tienich'] = dict(zip(alternative_names, weights))
            else:
                if 'tienich' in session['alternative_weights']:
                    del session['alternative_weights']['tienich']
        ...
        comparison_matrix_data = generate_alternative_comparison_matrix_data(alternatives, criteria.id, selected_honda_type_id, load_suggestions, session.get('tienich_comparison_values'))
    else:
