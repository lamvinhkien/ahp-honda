...
tlsd_bp = Blueprint("tlsd", __name__, url_prefix="/tlsd")
...
@tlsd_bp.route("/", methods=["GET", "POST"])
def tlsd_page():
    criteria_name = "Thời lượng sử dụng"
    criteria = Criteria.query.filter_by(name=criteria_name).first()
    honda_types = LaptopType.query.all()
    selected_laptop_type_id = request.form.get('selected_laptop_type_id') or request.args.get('selected_laptop_type_id') or session.get('tlsd_selected_laptop_type_id')
    selected_laptop_type = LaptopType.query.get(selected_laptop_type_id)
    alternatives = []
    if selected_laptop_type_id:
        session['tlsd_selected_laptop_type_id'] = selected_laptop_type_id
        alternatives = Alternatives.query.filter_by(laptop_type_id=selected_laptop_type_id).all()

    weights = session.get('tlsd_weights')
    cr = session.get('tlsd_cr')
    error = None
    submitted_values = session.get('tlsd_comparison_values', {})
    input_errors = {}
    ...
    if request.method == "POST" and criteria and selected_laptop_type:
        ...
        session['tlsd_comparison_values'] = submitted_values_from_form
        ...
        if valid_input:
            ...
            cr = cr_calculated
            session['tlsd_weights'] = weights
            session['tlsd_cr'] = cr
            session['alternative_weights'] = session.get('alternative_weights', {})
            if weights:
                session['alternative_weights']['tlsd'] = dict(zip(alternative_names, weights))
            else:
                if 'tlsd' in session['alternative_weights']:
                    del session['alternative_weights']['tlsd']
        ...
        comparison_matrix_data = generate_alternative_comparison_matrix_data(alternatives, criteria.id, selected_laptop_type_id, load_suggestions, session.get('tlsd_comparison_values'))
    else:
