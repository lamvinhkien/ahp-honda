...
chiphi_bp = Blueprint("chiphi", __name__, url_prefix="/chi-phi")
...
@chiphi_bp.route("/", methods=["GET", "POST"])
def chiphi_page():
    criteria_name = "Chi phí"
    criteria = Criteria.query.filter_by(name=criteria_name).first()
    honda_types = LaptopType.query.all()
    selected_laptop_type_id = request.form.get('selected_laptop_type_id') or request.args.get('selected_laptop_type_id') or session.get('chiphi_selected_laptop_type_id')
    selected_laptop_type = LaptopType.query.get(selected_laptop_type_id)
    alternatives = []
    if selected_laptop_type_id:
        session['chiphi_selected_laptop_type_id'] = selected_laptop_type_id
        alternatives = Alternatives.query.filter_by(laptop_type_id=selected_laptop_type_id).all()

    weights = session.get('chiphi_weights')
    cr = session.get('chiphi_cr')
    error = None
    submitted_values = session.get('chiphi_comparison_values', {})
    input_errors = {}
    ...
    if request.method == "POST" and criteria and selected_laptop_type:
        ...
        session['chiphi_comparison_values'] = submitted_values_from_form
        ...
        if valid_input:
            ...
            cr = cr_calculated
            session['chiphi_weights'] = weights
            session['chiphi_cr'] = cr
            session['alternative_weights'] = session.get('alternative_weights', {})
            if weights:
                session['alternative_weights']['chiphi'] = dict(zip(alternative_names, weights))
            else:
                if 'chiphi' in session['alternative_weights']:
                    del session['alternative_weights']['chiphi']
        ...
        comparison_matrix_data = generate_alternative_comparison_matrix_data(alternatives, criteria.id, selected_laptop_type_id, load_suggestions, session.get('chiphi_comparison_values'))
    else:
